...
df_train<-read.csv('C:/Users/Jose Chiramel/Desktop/UCD BA/EY Datascience/EY/train.csv')
df_test<-read.csv('C:/Users/Jose Chiramel/Desktop/UCD BA/EY Datascience/EY/test.csv')
alldat<-rbind(df_train, df_test)
write.csv(alldat,'alldat.csv')

df<-read.csv('alldat.csv')
...
ndf<-ndf%>%mutate(vels_new=(xvel_pre)^2+(yvel_pre)^2,vel_rat=xvel_pre/yvel_pre)

ndf<-ndf%>%group_by(hash)%>%mutate(velx_max=max(xvel_pre,na.rm = T),vely_max=max(yvel_pre,na.rm = T),velx_min=min(xvel_pre,na.rm = T),vely_min=min(yvel_pre,na.rm = T))
ndf<-ndf%>%group_by(hash)%>%mutate(vmeans_n=mean(vels_new,na.rm = T))

ndf$vmean[ndf$vmean=='NaN']=NA
ndf$vmean[ndf$vmean==1]=ndf$vmeans_n[ndf$vmean==1]
